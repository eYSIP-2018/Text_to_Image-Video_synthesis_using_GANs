...
"""
VideoFolderDataset
Description: Loads the dataset folder.
Constructor inputs:
...
__getitem__ 
input: 
    item: integer index from 0 to length of dataset.
outputs: 
    im: image
    ...
    def __init__(self, folder, cache, min_len=32):
        ...
        if cache is not None and os.path.exists(cache):
            with open(cache, 'rb') as f:
                self.images, self.lengths = pickle.load(f, encoding='latin1')
        else:
...
#                     pickle.dump((self.images, self.lengths), open(cache,"wb"))
...
"""
ImageDataset
Description: Transforms the image and returns the frames.

Constructor inputs:
    dataset: dataset as returned from VideoFolderDataset.
    transform: function to resize image, convert it to tensor and normalise the image.

__getitem__
input: 
    item: integer index from 0 to length of dataset.
output:
    dictionary of images(frames of video) and their categories
...
"""
VideoDataset
Description: Reduces the video to required size.

Constructor inputs:
    dataset: dataset as returned from VideoFolderDataset
    video_length: number of frames required
    every_nth: training videos are sampled using every nth frame
...
input:
    item: integer index from 0 to length of dataset.
output:
    dictionary of images after selecting the frames and their categories
...
Constructor inputs:
    dataset: dataset
    video_length: desired length of video
    every_nth: training videos are sampled using every nth frame
    transform: video transform function
